...
let rec visit_stmt (ast : Ast.stmt) (func_name : string)
  ...
  match ast.desc with
  | Compound stmt_list ->
    List.fold ~init:("", vars, types, num)
      ~f:(fun s stmt ->
          Scope.aggregate s
          @@ visit_stmt stmt func_name (Scope.get_vars s) (Scope.get_types s) (Scope.get_num s))
      stmt_list
  | Decl decl_list ->
    List.fold ~init:("", vars, types, num)
      ~f:(fun s decl ->
          Scope.aggregate s
          @@ visit_decl decl (Scope.get_vars s) (Scope.get_types s) (Scope.get_num s))
      decl_list
  | Expr expr -> visit_expr expr vars types num
  | If { cond; then_branch; else_branch; _ } ->
    visit_if_stmt cond then_branch else_branch func_name vars types num
  | For _ -> visit_for_stmt ast func_name vars types num
  | While _ -> visit_while_stmt ast func_name vars types num
  | Return (Some ret_expr) -> (
      match func_name with
      | "main" ->
        ("", vars, types, num) |> Scope.add_string "exit("
        |> Scope.extend ~f:(visit_expr ret_expr)
        |> Scope.add_string ")\n"
      | _ -> ("", vars, types, num) |> Scope.extend ~f:(visit_expr ret_expr))
  | Return None -> ("", vars, types, num) |> Scope.add_string "()"
  | _ ->
...
and visit_if_stmt (cond : Ast.expr) (then_branch : Ast.stmt)
  ...
  and else_str (vars : string VarMap.t)
      (types : (string * string) list VarMap.t) (num: int) =
    match else_branch with
    | Some e -> visit_stmt e func_name vars types num
    | None -> ("", vars, types, num)
  in
  match List.length mutated with
  | 0 ->
    (* TODO this might not work with some edge cases !!! *)
    ("", vars, types, num) |> Scope.add_string "if "
...
and visit_for_stmt (ast : Ast.stmt) (func_name : string)
  ...
  | For { cond; init; body; inc; _ } ->
    ...
    in
    Scope.add_string
      ("let rec forLoop" ^ (string_of_int num)^" (" ^ allMutated ^ ") (" ^ allVarNames ^ ") = if ")
      initStart
    ...
    |> Scope.add_string @@ "forLoop" ^ (string_of_int num)^" (" ^ allMutated ^ ") (" ^ allVarNames
                           ^ ") in \n"
    |> Scope.add_string @@ "let " ^ allMutated ^ " = forLoop" ^ (string_of_int num)^" (" ^ allMutated
                           ^ ") (" ^ allVarNames ^ ") in \n"
...
and visit_while_stmt (ast : Ast.stmt) (func_name : string)
  ...
  | While { cond; body; _ } ->
    ...
    let allMutated = String.concat ~sep:"," mutated in
    ("", vars, types, num)
    |> Scope.add_string @@ "let rec whileLoop" ^ (string_of_int num)^" (" ^ allMutated ^ ") = if "
    |> Scope.extend ~f:(visit_expr cond)
    |> Scope.add_string @@ " then "
    |> Scope.add_string @@ "(" ^ allMutated ^ ") \n"
    |> Scope.add_string @@ " else "
    |> Scope.extend ~f:(visit_stmt body func_name)
    |> Scope.add_string @@ "whileLoop" ^ (string_of_int num)^" (" ^ allMutated ^ ") in \n"
    |> Scope.add_string @@ "let " ^ allMutated ^ " = whileLoop" ^ (string_of_int num)^" (" ^ allMutated
                           ^ ") in \n"
  | _ -> failwith "never occurs"
      ...
      List.fold ~init:s
        ~f:(fun s item ->
            ...
            @@ parse_struct_field item ast.name (Scope.get_vars s)
              (Scope.get_types s) (Scope.get_num s))
        ast.fields)
  |> Scope.add_string " }"
...
and visit_decl (ast : Ast.decl) (vars : string VarMap.t)
    (types : (string * string) list VarMap.t) (num: int) : Scope.t =
  match ast.desc with
  | Function function_decl -> visit_function_decl function_decl vars types num
  | Var var_decl -> (
          ...
          | _ ->
            ...
            |> (fun s ->
                List.fold ~init:s
                  ~f:(fun s item ->
                      Scope.aggregate s
                      @@ visit_expr item (Scope.get_vars s) (Scope.get_types s) (Scope.get_num s)
                      |> Scope.add_string "; ")
                  expr_list)
            |> Scope.add_string "]"))
  | _ -> visit_expr ast vars types num

and visit_empty_init (var_decl: Clang.Ast.var_decl_desc) (vars : string VarMap.t)
    (types : (string * string) list VarMap.t) (num: int) : Scope.t =
  ...
  if is_array_type var_decl.var_type then
    ...
    match get_array_type var_decl.var_type with
    | "char" -> ("", vars, types, num)
                |> Scope.add_var var_decl.var_name var_type_name
                |> Scope.add_string @@ "let " ^ var_name ^ " : " ^ var_type_name
                                       ^ " = create_empty_char_list " ^ n ^ " in \n"
    | "int" -> ("", vars, types, num)
               |> Scope.add_var var_name var_type_name
               |> Scope.add_string @@ "let " ^ var_name ^ " : " ^ var_type_name
                                      ^ " = create_empty_int_list " ^ n ^ " in \n"
    | "float" -> 
      ("", vars, types, num)
      |> Scope.add_var var_decl.var_name var_type_name
      |> Scope.add_string @@ "let " ^ var_decl.var_name ^ " : " ^ var_type_name
                             ^ " = create_empty_float_list " ^ n ^ " in \n"
    | _ -> failwith "unsupported type"
  else
    ("", vars, types, num) |> Scope.add_var var_name var_type_name
...
and visit_expr (ast : Ast.expr) (vars : string VarMap.t)
  ...
  | IntegerLiteral i -> (
      match i with
      | Int value ->
        ("", vars, types, num) |> Scope.add_string (Int.to_string value ^ " ")
      | _ -> assert false)
  | FloatingLiteral f -> (
      match f with
      | Float value ->
      ...
      | _ ->
        ...
        |> (fun s ->
            List.fold ~init:s
              ~f:(fun s arg ->
                  Scope.aggregate s
                  @@ visit_expr arg (Scope.get_vars s) (Scope.get_types s) (Scope.get_num s))
              args)
        |> Scope.add_string ")")
  | _ ->
...
let parse (source : string) : string =
  ...
  Scope.get_string
    (List.fold ~init:Scope.empty
       ~f:(fun s item ->
           Scope.aggregate s
           @@ visit_decl item (Scope.get_vars s) (Scope.get_types s) (Scope.get_num s))
       ast.desc.items)
