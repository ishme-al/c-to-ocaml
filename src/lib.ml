...
let parse_func_params (ast : Ast.function_decl) : string =
  let parse_param = fun (acc: string) (p: Ast.parameter) ->
  ...
  | Some params when params.variadic -> failwith "Variadic functions are not supported"
  | Some params -> (
    List.fold ~f:(parse_param) (params.non_variadic) ~init:""
  )
  | None -> ""
...
let visit_function_decl (ast : Ast.function_decl) (out : Out_channel.t) : unit =
  match ast.name with
  | IdentifierName name -> Out_channel.output_string out @@ "let " ^ name ^ " " ^ (parse_func_params ast) ^ ": " ^ (parse_func_return_type ast) ^ " = \n"; 
...
let visualize_ast (ast:Translation_unit.t) (out : Out_channel.t) : unit =
  let foo =
    ...
    | _ -> failwith "failure in visualize_ast" in
