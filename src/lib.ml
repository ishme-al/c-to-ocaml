...
and visit_for_stmt (ast : Ast.stmt) (func_name : string)
  ...
  | For { cond; init; body; inc; _ } -> (
      ...
      let initStart = match init with 
        ...
        | None -> start in 
      Scope.add_string ("let rec forLoop (" ^ allMutated ^ ") (" ^ allVarNames ^ ") = if ") initStart
...
and get_function_name (ast : Ast.function_decl)  : string =
  match ast.name with
  | IdentifierName "main" ->
    "main"
  | IdentifierName name ->
...
and get_struct_name (ast : Ast.record_decl)  : string =
  ast.name
...
and get_decl_names (ast : Ast.decl) 
  ...
  | RecordDecl struct_decl -> get_struct_name struct_decl
  (* | Field { name; qual_type; _ } ->
      Scope.add_string
        (name ^ ": " ^ parse_qual_type qual_type ^ "; ")
        ("", vars, types) *)
