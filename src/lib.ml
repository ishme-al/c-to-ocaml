...
let rec visit_stmt (ast : Ast.stmt) (func_name : string)
  ...
  | For _ -> visit_for_stmt ast func_name vars types
  | While _ -> visit_while_stmt ast func_name vars types
...
and visit_for_stmt (ast : Ast.stmt) (func_name : string)
  ...
  | For { cond; init; body; inc; _ } ->
    ...
    in
    Scope.add_string
      ("let rec forLoop (" ^ allMutated ^ ") (" ^ allVarNames ^ ") = if ")
      initStart
    ...
    |> Scope.add_string @@ "forLoop (" ^ allMutated ^ ") (" ^ allVarNames
                           ^ ") in \n"
    |> Scope.add_string @@ "let " ^ allMutated ^ " = forLoop (" ^ allMutated
                           ^ ") (" ^ allVarNames ^ ") in \n"
...
and visit_while_stmt (ast : Ast.stmt) (func_name : string)
  ...
  | While { cond; body; _ } ->
    ...
    |> Scope.add_string @@ "let " ^ allMutated ^ " = whileLoop (" ^ allMutated
                           ^ ") in \n"
  | _ -> failwith "never occurs"
...
and visit_empty_init (var_decl: Clang.Ast.var_decl_desc) (vars : string VarMap.t)
  ...
  if is_array_type var_decl.var_type then
    ...
    ("", vars, types) |> Scope.add_var var_name var_type_name
