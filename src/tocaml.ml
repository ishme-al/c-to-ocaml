...
let format (input_name : string) (output_file : string) (source : string) :
    string =
  (* print_endline source; *)
  match
    Conf.default
    |> Translation_unit.parse_and_format Syntax.Use_file ~output_file
         ~input_name ~source
  with
...
let command =
  Command.basic ~summary:"Transpile c to ocaml code"
    ...
    (let%map_open.Command input = anon ("[input-file]" %: string)
