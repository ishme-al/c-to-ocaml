...
module Scope = struct
  ...
  let rec set_at_index (lst: 'a list) (index:int) (value: 'a) : 'a list =
    ...
    | hd :: tl ->
      ...
      let create_empty_float_list n =
        ...
        "
                   , 
                   VarMap.empty, VarMap.empty, 0)

  let aggregate (scope1 : t) (scope2 : t) : t =
    let str, _, _, num1 = scope1 in
    let str', var_map', type_map', num2 = scope2 in
    (str ^ str', var_map', type_map', num1 + num2) 

  let add_string (s : string) (scope : t) : t =
    let str, var_map, type_map, num = scope in
    (str ^ s, var_map, type_map, num + 1)

  let add_var (name : string) (s : string) (scope : t) : t =
    let str, var_map, type_map, num = scope in
    let var_map' = Map.update var_map name ~f:(fun _ -> s) in
    (str, var_map', type_map, num)
  ...
  let get_string (scope : t) : string =
    let str,_,_,_ = scope in
    str

  let get_vars (scope : t) : string VarMap.t =
    let _, map, _, _ = scope in
    map


  let get_types (scope : t) : (string * string) list VarMap.t =
    let _, _, map, _ = scope in
    map

  let get_num (scope : t) : int =
    let _, _, _, num= scope in
    num 
  ...
  let extend ~(f : string VarMap.t -> (string * string) list VarMap.t -> int -> t) (scope : t) : t =
    f (get_vars scope) (get_types scope) (get_num scope) |> aggregate scope
end